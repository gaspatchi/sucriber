...
class Tarantool {
	...
	async addSubscriptions(user_id, type, id) {
		let connection = await this._connect();
		let response = await connection.call("addSubscriptions", user_id, type, id);
		return response;
	}

	async deleteSubscriptions(user_id, type, id) {
		let connection = await this._connect();
		let response = await connection.call("deleteSubscriptions", user_id, type, id);
		return response;
	}
